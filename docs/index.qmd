...
This report shows population-level results of relative density, occurrence, and length frequency distributions for ecologically important, economically valuable, and fish species of interest to marine spatial planners and resource managers.
...
spp_list_STX <- STTSTJ$taxonomic_data %>%
  filter(SPECIES_CD %in% target_species) %>%
  select(SPECIES_CD, COMNAME, SCINAME) %>%
  distinct() %>%
  arrange(match(SPECIES_CD, target_species)) %>%
